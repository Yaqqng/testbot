...
remnawave = RemnawaveClient(settings.remnawave_base_url, settings.remnawave_api_key)
...
async def start_handler(message: Message) -> None:
    ...
    sub_ok = await check_channel_subscription(user.user_id)

    if not sub_ok:
        await message.answer(
            f"Чтобы пользоваться ботом, подпишитесь на канал: {settings.main_channel}\n"
...
async def check_sub_handler(callback: CallbackQuery) -> None:
    ok = await check_channel_subscription(callback.from_user.id)
    if ok:
        await callback.message.answer("Подписка подтверждена ✅")
    else:
        await callback.message.answer(f"Вы ещё не подписаны на {settings.main_channel}")
...
async def buy_handler(callback: CallbackQuery) -> None:
    if not await check_channel_subscription(callback.from_user.id):
        await callback.message.answer("Сначала подпишитесь на канал и подтвердите подписку.")
        await callback.answer()
        return
    ...
    try:
        ...
        remnawave_id = str(created.get("id") or created.get("uuid") or "")
